...
import 'providers/final_search_providers.dart';

/// Final Arama Tab
/// Ürün koduna ve tarihe göre final kontrol kayıtlarını arar
class FinalSearchTab extends ConsumerStatefulWidget {
  const FinalSearchTab({super.key});

  @override
  ConsumerState<FinalSearchTab> createState() => _FinalSearchTabState();
}

class _FinalSearchTabState extends ConsumerState<FinalSearchTab> {
  ...
  void _performSearch() {
    ...
    ref
        .read(finalSearchStateProvider.notifier)
        .searchFinalRecords(
  ...
  void _clearSearch() {
    ...
    ref.read(finalSearchStateProvider.notifier).clear();
  }

  @override
  Widget build(BuildContext context) {
    final searchState = ref.watch(finalSearchStateProvider);
  ...
  Widget _buildStatisticsCards(List results) {
    ...
    final totalReject = results.fold<int>(
      0,
      (sum, item) => sum + (item.rejectQuantity as int),
    );
    ...
    return Row(
      children: [
        ...
        Expanded(
          child: _buildStatCard(
            'Toplam Kontrol',
            (totalProduction + totalReject).toString(),
            LucideIcons.files,
            AppColors.primary,
          ),
        ),
              ...
              rows: results.map((result) {
                return DataRow(
                  cells: [
                    ...
                    DataCell(
                      Text(
                        result.rejectQuantity.toString(),
                        style: const TextStyle(
